...
final class JsltApplicationSpec extends AnyFlatSpec with Matchers {
  ...
  it should "rename" in {
    ...
    // G I V E N
    val jsonNodeInput = mapper.readTree(msg)
    val jslt = Parser.compileString("""if (.features.location.speed) { "caracteristicas": { "localizacion" : { "velocidad" : .features.location.speed } } }""".stripMargin)
  ...
  it should "rename multiples keys" in {
    ...
    // G I V E N
    val jsonNodeInput = mapper.readTree(msg)
    val jslt = Parser.compileString("""if (.features.location.speed and .features.location.latitude and .features.location.longitude) { "caracteristicas": { "localizacion" : { "velocidad" : .features.location.speed } + { "latitud": .features.location.latitude} + { "longitud": .features.location.longitude} } }""".stripMargin)
  ...
  it should "exclude" in {
    ...
    // T H E N
    val expected = mapper.readTree("""{"id":"3771a4d1-477f-459c-9b64-90207e486992","timestamp":"2019-03-18T15:28:07.000Z","server":{"timestamp":"2019-03-18T15:28:10.053Z"},"attributes":{"device":{"manufacturer":"Pointer","model":"PointerCompactFleet","tenantId":848860983001616384,"identifier":"256111"},"sensors":{"di4":{"feature":"door"}}}}""")
    actual shouldBe expected
  ...
  it should "exclude key" in {
    ...
    // T H E N
    val expected = mapper.readTree("""{"id":"3771a4d1-477f-459c-9b64-90207e486992","timestamp":"2019-03-18T15:28:07.000Z","server":{"timestamp":"2019-03-18T15:28:10.053Z"},"attributes":{"device":{"manufacturer":"Pointer","model":"PointerCompactFleet","tenantId":848860983001616384,"identifier":"256111"},"sensors":{"di4":{"feature":"door"}}},"features":{"gnss":{"type":"Gps","precision":"Unknown","satellites":4},"di4":{"status":true},"engine":{"ignition":{"status":false}},"mainpower":{"voltage":11.443},"network":{"type":"Gprs"}}}""")
    actual shouldBe expected
