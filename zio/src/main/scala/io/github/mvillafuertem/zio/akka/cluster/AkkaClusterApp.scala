...
object AkkaClusterApp extends App {
  ...
  override def run(args: List[String]): ZIO[zio.ZEnv, Nothing, Int] = program(ActorSystem("Chat"))

  def program(actorSystem: ActorSystem): ZIO[Console, Nothing, Int] =
    for {
      p <- Managed
        .make(Task(actorSystem))(sys => Task(sys.terminate()).ignore)
        .use(
          actorSystem =>
            for {
