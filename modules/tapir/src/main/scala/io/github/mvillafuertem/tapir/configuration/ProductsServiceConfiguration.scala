...
trait ProductsServiceConfiguration extends InfrastructureConfiguration {
  ...
  def httpServer(actorSystem: ActorSystem[_]): ZIO[Any, Throwable, Unit] = {
    ...
    val eventualBinding                                = Http()(untypedSystem).newServerAt(productsConfigurationProperties.interface, productsConfigurationProperties.port).bind(SwaggerApi.route ~ new ProductsApi(new SlickProductsRepository() {
