 ...
 */
final class NumbersIT extends NumbersConfigurationIT with EventConfiguration with AnyFlatSpecLike with Matchers with ScalaFutures with BeforeAndAfterAll {
  ...
  private val msg: String = scala.io.Source
    .fromURL(getClass.getResource("/event.json"))
    .getLines()
    .mkString
    .replaceAll("[\\n\\s]", "")
  ...
  def produce(): Unit = {
    ...
    val eventualDone = Source(LazyList.from(1 to 1)).map { s =>
      Thread.sleep(1)
      log.info("{}", msg)
      msg
    }.map { value =>
